...
class Connection:
    ...
    async def add_episodes(self,episodes: List[Dict[str, Union[str, Dict[str, str], EpisodeType]]]):
        try:
            await self.graphiti.build_indices_and_constraints()
            for i, episode in enumerate(episodes):
                await self.graphiti.add_episode(
                    name=f'Freakonomics Radio {i}',
                    episode_body=episode['content'] if isinstance(episode['content'], str) else json.dumps(
                        episode['content']),
                    source=episode['type'],
